...
def test_main_passes_opt_in_remote_performance_diagnostics_to_game(monkeypatch) -> None:
    calls = []
    ...
    assert calls == [
        {
            "debug_mode": False,
            "remote_playtest_controls": True,
            "remote_playtest_performance_diagnostics": True,
        },
        "main_menu",
        "cleanup",
    ]
...
def test_main_passes_fullscreen_flag_to_game(monkeypatch) -> None:
    calls = []
    ...
    monkeypatch.setattr(pygame_game, "PygameGame", FakeGame)
    monkeypatch.setattr(pygame_game, "install_signal_handlers", lambda: None)
    monkeypatch.setattr(pygame_game.sys, "argv", ["game_pygame.py", "--fullscreen"])

    assert pygame_game.main() == 0
    assert calls == [{"debug_mode": False, "fullscreen": True}, "main_menu", "cleanup"]
